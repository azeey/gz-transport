...
class MultipleThreads : public testing::Test
{
  // protected:
  // void SetUp() override
  // {
...
TEST_F(MultipleThreads, PubSub)
{
  auto testThread = []
  {
    std::mutex cbMutex;
    std::condition_variable cv;
    bool receivedPub{false};
    gz::transport::Node node;
    std::stringstream topic;
    topic << "/chatter_" << std::this_thread::get_id();
    auto pub = node.Advertise<msgs::Vector3d>(topic.str());
    msgs::Vector3d msg;
    msg.set_x(1.0);
    msg.set_y(2.0);
    msg.set_z(3.0);
    ...
    auto cb = std::function(
        [&](const msgs::Vector3d &_data)
        {
          EXPECT_EQ(msg.x(), _data.x());
          std::lock_guard<std::mutex> lk(cbMutex);
          receivedPub = true;
          cv.notify_one();
        });

    node.Subscribe(topic.str(), cb);
    EXPECT_TRUE(pub.Publish(msg));

    std::unique_lock lk(cbMutex);
    cv.wait(lk, [&] { return receivedPub; });
    EXPECT_TRUE(receivedPub);
  };
